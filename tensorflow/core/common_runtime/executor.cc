...
#include <map>
//static std::map<std::string, int> tf_mp;
namespace tensorflow {
//std::map<string, int> tf_mp;
namespace {
 ...
 private:
  ...
  class TaggedNodeReadyQueue {
   public:
    ...
    void insert_front(TaggedNode node){
      ready_.insert(ready_.begin() + front_index_, node);
    }    
...
void ExecutorState::ScheduleReady(const TaggedNodeSeq& ready,
  ...
  //std::sort(ready.begin(), ready.end(), [this](TaggedNode& t1, TaggedNode& t2){
  //  return name2order[t1.node_item->kernel->name()] < name2order[t2.node_item->kernel->name()];
  //});

  TaggedNodeSeq sends;
  TaggedNodeSeq idens;
  TaggedNodeSeq varis;
  TaggedNodeSeq consts;
  //TaggedNodeSeq recvs;
  //TaggedNodeSeq norms;
  ...
  for (auto& tagged_node : ready) {
    ...
    if (tagged_node.is_dead || !item.kernel->IsExpensive()) {
      ...
      //inline_ready->push_back(tagged_node);
      if(tagged_node.node_item->kernel->type_string() == "_Send"){
        //printf("%s\n", tagged_node.node_item->kernel->name().c_str());
        sends.push_back(tagged_node);
      //}else if(tagged_node.node_item->kernel->type_string()=="_Recv"){
      //  recvs.push_back(tagged_node);
      }else if(tagged_node.node_item->kernel->type_string()=="Identity"){
        idens.push_back(tagged_node);
      }else if(tagged_node.node_item->kernel->type_string() == "VariableV2"){
        varis.push_back(tagged_node);
      }else if(tagged_node.node_item->kernel->type_string() == "Const"){
        consts.push_back(tagged_node);
      }//else inline_ready->push_back(tagged_node);
      else if( (tagged_node.node_item->kernel->type_string() == "Conv2DBackpropFilter") 
      || (tagged_node.node_item->kernel->type_string() == "Conv2DBackpropInput")
      || (tagged_node.node_item->kernel->type_string() == "FusedBatchNormGradV3")
      || (tagged_node.node_item->kernel->type_string() == "ReluGrad")
      || (tagged_node.node_item->kernel->type_string() == "AddN")
      || (tagged_node.node_item->kernel->type_string() == "MaxPoolGrad")
      ){
        inline_ready->push_back(tagged_node);
      }
      else //inline_ready->push_back(tagged_node);
      {    
        //norms.push_back(tagged_node);
          inline_ready->insert_front(tagged_node);
          //printf("%s, %s\n", 
          //  tagged_node.node_item->kernel->type_string().c_str(),
          //  inline_ready->front().node_item->kernel->type_string().c_str()
          //);
      }
    } else {
  ...
  std::sort(consts.begin(), consts.end(), [this](TaggedNode & t1, TaggedNode & t2){
    return impl_->mp.find(t1.node_item->kernel->name())->second < impl_->mp.find(t2.node_item->kernel->name())->second;
  });
  for(auto& tagged_node: consts){
    inline_ready->push_back(tagged_node); 
  }
  std::sort(idens.begin(), idens.end(), [this](TaggedNode & t1, TaggedNode & t2){
    return impl_->mp.find(t1.node_item->kernel->name())->second < impl_->mp.find(t2.node_item->kernel->name())->second;
  });
  for(auto& tagged_node: idens){
   inline_ready->push_back(tagged_node); 
  }
  std::sort(varis.begin(), varis.end(), [this](TaggedNode & t1, TaggedNode & t2){
  //  return mp[t1.node_item->kernel->name()] < mp[t2.node_item->kernel->name()];
    return impl_->mp.find(t1.node_item->kernel->name())->second < impl_->mp.find(t2.node_item->kernel->name())->second;
  });
  for(auto& tagged_node: varis){
    inline_ready->push_back(tagged_node); 
  }
  std::sort(sends.begin(), sends.end(), [this](TaggedNode & t1, TaggedNode & t2){
    //return mp[t1.node_item->kernel->name()] < mp[t2.node_item->kernel->name()];
    return impl_->mp.find(t1.node_item->kernel->name())->second < impl_->mp.find(t2.node_item->kernel->name())->second;
  });
  for(auto& tagged_node: sends){
   inline_ready->push_back(tagged_node); 
  }
  //std::sort(recvs.begin(), recvs.end(), [this](TaggedNode & t1, TaggedNode & t2){
  //  return name2order[t1.node_item->kernel->name()] < name2order[t2.node_item->kernel->name()];
  //});
  //for(auto& tagged_node: recvs){
  //  inline_ready->push_back(tagged_node); 
    //runner_(std::bind(&ExecutorState::Process, this, tagged_node,
    //                      scheduled_nsec));
  //}
  
  //std::sort(norms.begin(), norms.end(), [this](TaggedNode & t1, TaggedNode & t2){
  //  return name2order[t1.node_item->kernel->name()] > name2order[t2.node_item->kernel->name()];
  //});
  //for(auto& tagged_node: norms){
  //  inline_ready->insert_front(tagged_node); 
    //runner_(std::bind(&ExecutorState::Process, this, tagged_node,
    //                      scheduled_nsec));
  //}
